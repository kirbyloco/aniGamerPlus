...
import Config
from Anime import Anime, Exp, TryTooManyTimeError
...
def insert_db(anime):
    ...
    try:
        ...
        err_print(anime_dict['sn'], 'ＤＢ错误', 'title=' +
                  anime_dict['title'] + ' 数据已存在！' + str(e), status=1)
...
def worker(sn, sn_info, realtime_show_file_size=False):
    ...
    if settings['upload_to_server']:
        ...
        try:
            ...
            # 兜一下各种奇奇怪怪的错误
            err_print(sn, '上傳異常', '發生未知錯誤, 從任務列隊中移除, 等待下次更新重試: ' +
                      str(e), status=1)
        ...
        if sn_dict[sn]['mode'] == 'all':
            ...
            try:
                ...
                if anime.get_sn() == latest_sn:
                    ...
                    if new_anime['failed']:
                        err_print(latest_sn, '更新狀態',
                                  '更新數據失敗, 跳過等待下次檢查', status=1)
                        continue
                    new_anime = new_anime['anime']
                insert_db(new_anime)
...
def __download_only(sn, dl_resolution='', dl_save_dir='', realtime_show_file_size=False, classify=True):
    ...
    try:
        if dl_resolution:
            anime.download(dl_resolution, dl_save_dir,
    ...
    while anime.video_size < 10:
        if err_counter >= 3:
            err_print(sn, '終止任務', 'title=' + anime.get_title() +
                      ' 任務失敗達三次! 終止任務!', status=1)
            thread_limiter.release()
            return
        else:
            err_print(sn, '任務失敗', 'title=' + anime.get_title() +
                      ' 10s后自動重啓,最多重試三次', status=1)
            ...
            try:
                if dl_resolution:
                    anime.download(
                        dl_resolution, dl_save_dir, realtime_show_file_size=realtime_show_file_size, classify=classify)
                else:
                    anime.download(settings['download_resolution'], dl_save_dir,
                                   realtime_show_file_size=realtime_show_file_size, classify=classify)
            except BaseException as e:
...
def __cui(sn, cui_resolution, cui_download_mode, cui_thread_limit, ep_range,
    ...
    if cui_download_mode == 'single':
        ...
        for anime_sn in bangumi_list:
            if get_info:
                task = threading.Thread(
                    target=__get_info_only, args=(anime_sn,))
            else:
                task = threading.Thread(target=__download_only, args=(
        ...
        if get_info:
            ...
            print('所有下載任務已添加至列隊, 共 ' + str(tasks_counter) +
                  ' 個任務, ' + '執行緒數: ' + str(cui_thread_limit) + '\n')
        ...
        for ep in ep_range:
            if ep in bangumi_ep_list:
                if get_info:
                    ...
                    a = threading.Thread(target=__download_only, args=(
                        episode_dict[ep], cui_resolution, cui_save_dir, realtime_show_file_size))
                a.setDaemon(True)
                ...
                if get_info:
                    print(
                        '添加查詢列隊: sn=' + str(episode_dict[ep]) + ' 《' + anime.get_bangumi_name() + '》 第 ' + ep + ' 集')
                else:
                    print(
                        '添加任务列隊: sn=' + str(episode_dict[ep]) + ' 《' + anime.get_bangumi_name() + '》 第 ' + ep + ' 集')
            else:
                err_print(0, '《' + anime.get_bangumi_name() +
                          '》 第 ' + ep + ' 集不存在!', status=1, no_sn=True)
        print('所有任務已添加至列隊, 共 ' + str(tasks_counter) + ' 個任務, ' +
        ...
        for i in ep_range:
            ...
            if get_info:
                ...
                # True 是实时显示文件大小, 仅一个下载任务时适用
                anime.download(cui_resolution, cui_save_dir,
                               realtime_show_file_size=True, classify=classify)
        ...
        if get_info:
            ...
            for sn in queue.keys():  # 遍历任务列队
                ...
                task = threading.Thread(target=worker, args=(
                    sn, queue[sn], realtime_show_file_size))
                task.setDaemon(True)
                thread_tasks.append(task)
                task.start()
                err_print(sn, '加入任务列隊')
    ...
    if settings['use_gost']:
        ...
        if check_gost.stderr.readlines():  # 查找 gost 是否已放入系统 path
            ...
            if not os.path.exists(gost_path):
                err_print(0, '當前代理使用擴展協議, 需要使用gost, 但是gost未找到',
                          status=1, no_sn=True)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:  # 支持命令行使用
        ...
        parser.add_argument('--resolution', '-r', type=int,
                            help='指定下載清晰度(數字)', choices=[360, 480, 540, 576, 720, 1080])
        ...
        parser.add_argument('--thread_limit', '-t',
                            type=int, help='最高并發下載數(數字)')
        ...
        if (arg.download_mode not in ('list', 'multi', 'sn-list')) and arg.sn is None:
            err_print(0, '參數錯誤', '非 list/multi 模式需要提供 sn ',
                      no_sn=True, status=1)
            sys.exit(1)
        ...
        if arg.current_path:
            ...
            err_print(0, info + settings['bangumi_dir'],
        ...
        if not arg.resolution:
            ...
            if arg.download_mode in ('sn-list', 'list'):
                err_print(0, '無效參數:', 'list 及 sn-list 模式無法通過命令行指定清晰度',
                          1, no_sn=True, display_time=False)
                resolution = settings['download_resolution']
    ...
    while True:
        ...
        info = '本次更新添加了 ' + str(new_tasks_counter) + ' 個新任務, 目前列隊中共有 ' + \
            str(len(processing_queue)) + ' 個任務'
